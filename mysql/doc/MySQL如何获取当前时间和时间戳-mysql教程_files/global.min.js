var is_login, is_commented, ue, article_status, timestamp_version, learn, videolearn, videoHtml, isPostLevelUser,WorkArticleId, BlogArticleId, BlogArticleSetIt,isSpaceExe,forget_pop;
$(function() {
    layui.use(["layer", "form", "element", "util"],function() {
        ...
        $(document).on("click", "#js-note-submit", function() {
            ...
            isPostNote = true;
            $.post("/course/add_note.html", {learn: learn,content: $("#js-note-textarea").val()}, function(result) {
                if (result.status == 1) {
                    layer.msg(result.msg, {icon: 1});
                    $(".js-modal-close").trigger("click");
        ...
        if ($(".js-editor-tab .editor-view .ui-tabs-active").size() > 0) {
            ...
            $("#J_Commit").click(function() {
                ...
                if ((tmp_code.indexOf("imagedestroy") >= 0 || tmp_code.indexOf("imagecopyresampled") >= 0 || tmp_code.indexOf("imagecreatetruecolor") >= 0 || tmp_code.indexOf("imagecreatefrom") >= 0) && tmp_code.indexOf("header") >= 0) {
                    ...
                    $.post("https://work.php.cn/compile.php", {code: tmp_code,language: runcode},function(data) {
                        ifrw.document.open();
        ...
        if ($(".php-video #myVideo").size() > 0) {
            ...
            if (learn) {
                ...
                if (!VideoUrl) {
                    layer.alert("网络错误，请刷新后重试", {icon: 2});return false
                }
                $.ajax({
                    ...
                    async: false,
                    dataType: "json",
                    data: {"learn": learn, "VideoUrl": VideoUrl},
                    success: function(result) {
                        if (result.status == 1) {
                            if (result.type == 1) {
                                if (!result.given) {
                                    layer.alert("内部视频 开通联系微信：phpcn01", {icon: 2});return false
                                } else {
                                    if (result.given == 2) {
                                        layer.msg("请使用php中文网浏览器打开", {icon: 2, time: 5000});return false
                                    } else {
                            ...
                            if (result.type == 2) {}
                            if (result.login != 1) {
                                is_login = 0;$(document).trigger("api.loginpopbox");return false;
                                /*layer.alert("您未登陆,登陆后可继续观看视频", {icon: 2,skin: "layui-layer-molv",title:false,yes:function() {
                                    $(document).trigger("api.loginpopbox")
                                    },cancel:function(){$(document).trigger("api.loginpopbox");}
                                });*/
                            }
                            if (!result.video_path) {
                                video_down();return false
                            }
                            ...
                            $.getScript("/static/js/jquery.cookie.js", function() {
                                ...
                                var adTimer = setInterval(function() {
                                    ...
                                    if (adTime <= 0) {
                                        ...
                                        setTimeout(function() {clearInterval(adTimer)}, 300)
                                    }
                                }, 1000)
                            })
                        } else {
            ...
            if (videolearn) {
                ...
                $.ajax({
                    ...
                    async: false,
                    dataType: "json",
                    data: {"learn": videolearn, "VideoUrl": VideoUrl},
                    success: function(result) {
                        if (result.status == 1) {
                            if (result.login != 1) {
                                is_login = 0;$(document).trigger("api.loginpopbox");return false;
                            ...
                            if (result.ispay == 1) {
                                ...
                                layer.msg(result.msg, {icon: 2, skin: "layui-layer-molv"}, function() {
                                    layer.closeAll()
                                })
                            }
                            return false
        ...
        if ($(".left-column").size() > 0) {
            ...
            $("#J_Commit").click(function() {
                ...
                var ifrw = (ifr.contentWindow) ? ifr.contentWindow: (ifr.contentDocument.document) ? ifr.contentDocument.document: ifr.contentDocument;
                $.post("https://work.php.cn/compile.php", {code: tmp_code, language: runcode}, function(data) {
                    ifrw.document.open();ifrw.document.write(data);ifrw.document.close()
                })
            });
            if ($("body").width() < 767) {
        ...
        $(".article_publish").click(function() {
            ...
            if (!new RegExp("[\\u4E00-\\u9FFF]+", "g").test(content) || content.length < 6) {
                layer.msg("请输入6个字以上的内容", {icon: 2});return false
            }
            var id = $(this).data("id");
            if (!content) {
                layer.msg("内容不可为空", {icon: 2});return false
            }
            $.post("/article/reply_publish.html", {id: id,content: content},function(result) {
                if (result.code == 1) {
        ...
        $(".courseICollection").click(function() {
            ...
            var id = $(this).data("id");
            if(IscourseICollection){return false;}
        ...
        $(".user_order_close:not(.running)").click(function() {
            ...
            isPost = true;
            layer.confirm("是否删除该订单", {title: "我的订单", skin: "layui-layer-molv", btn: ["确定", "取消"], shade: false},function() {
                $.post("/user/order_del.html", {id: id},function(res) {
                    if (res.code == 1) {
                        layer.msg(res.msg, {icon: 1,time: 2000},function() {window.location.reload()})
                    } else {
                        layer.msg(res.msg, {icon: 2});isPost = false
                    }
                },"json")
            })
        });
        ...
        $(".user-js-add-follow").click(function() {
            ...
            var id = $(this).data("id");
            $.post("/member/follows.html", {id: id},function(res) {
                if (res.code == 1) {
        ...
        if ($(".pinBoxContainer").size() > 0) {
            ...
            $.getScript("/static/js/jquery.pinBox.min.js",function() {
        ...
        if ($("#aritcle_detail_new").size() > 0) {
            $.getScript("/static/js/scrollfix.min.js",function() {
                $("#aritcle_detail_new").scrollFix({
                    distanceTop: 0,zIndex:9999
                })
            })
        }
        $(".tool-banner-operation .collection:not('.running')").click(function() {
            ...
            that.addClass(".running");
            $.post("/subject/subject_collection.html", {type: 0,id: id},function(result) {
                if (result.code == 1) {
                    layer.msg(result.msg, {icon: 1,offset: "t"},function() {
                        window.location.reload()
                    })
                } else {
                    layer.msg(result.msg, {icon: 2,offset: "t"});
                    that.removeClass("running")
                }
            },"json")
        });
        $(".tool-banner-operation .good:not('.running')").click(function() {
            ...
            var id = that.parent(".php-course-info-botton").data("id");
            that.addClass(".running");
            $.post("/subject/subject_collection.html", {type: 1,id: id},function(result) {
                if (result.code == 1) {
                    layer.msg(result.msg, {icon: 1,offset: "t"}, function() {
                        window.location.reload()
                    })
        ...
        $(".tool-banner-operation .oppose:not('.running')").click(function() {
            ...
            var id = that.parent(".php-course-info-botton").data("id");
            that.addClass(".running");
            $.post("/subject/subject_collection.html", {type: 2, id: id}, function(result) {
                if (result.code == 1) {
                    layer.msg(result.msg, {icon: 1, offset: "t"}, function() {
                        window.location.reload()
                    })
                } else {
                    layer.msg(result.msg, {icon: 2, offset: "t"});that.removeClass("running")
                }
            },
            "json")
        });
        if ($(".domain-box").size() > 0) {
            $(document).on("click", ".open_blog_btn:not(.running)", function() {
                var that = $(this);
                var subdomain = $.trim($("#search-input").val()).toLocaleLowerCase();
                var id = that.data("id");
                $("#search-input").val(subdomain);
                if (subdomain.length == 0) {
                    layer.msg("请填url别名", {icon: 2});$("#search-input").focus();return false
                }
                if (subdomain.length < 6) {
                    layer.msg("url别名的长度不能少于6个字符", {icon: 2});$("#search-input").focus();return false
                }
                if (/^[\d]+$/.test(subdomain)) {
                    layer.msg("url别名不允许纯数字", {icon: 2});$("#search-input").focus();return false
                }
                if (!/^[a-z0-9]{6,10}$/.test(subdomain)) {
                    layer.msg("url别名只允许由小写字母和数字组成,长度不能超过10", {icon: 2});$("#search-input").focus();return false
                }
                $(this).addClass("running");
                layer.confirm("博客开通后url别名不可更改，是否确定开通？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
                    $.post("/member/" + id + "/blog.html", {subdomain: subdomain}, function(result) {
                        if (result.status) {
                            layer.msg("开通成功,正在为您跳转博客", {icon: 1}, function() {location.href = result.url})
                        } else {
                            layer.msg(result.msg, {icon: 2});that.removeClass("running");return false
                        }
                    })
                })
            })
        ...
        $(document).on("click", ".video_task_right_icon", function() {
            $(".task_rights").hide();
        ...
        $(".dic_search_form").bind("input porpertychange", function() {
            var this_val = $.trim($(this).val());
            if (this_val) {
                $.get("/dic/search_ajax", {val: this_val}, function(result) {
                    if (result) {
        ...
        if ($(".php-ask .report").size() > 0) {
            $(".php-ask .report").click(function() {
                if ($("#login").size() > 0) {$(document).trigger("api.loginpopbox");return false}
                var id = $(this).data("id");
        ...
        $(".manual-search-button").click(function() {
            ...
            if (!val) {
                layui.msg("请输入关键词", {icon: 2});return false
            }
            $.post("/manual_search/", {val: val}, function(res) {
                if (res) {
                    $(".route-manual-search-data").empty().append(res.msg)
                } else {
                    layui.msg("网络错误，请稍后重试", {icon: 2})
                }
            }, "json")
        });
        $(".manual_search_form").bind("input porpertychange", function() {
            var this_val = $.trim($(this).val());
            if (this_val) {
                $.get("/route/search_ajax", {val: this_val}, function(result) {
                    if (result) {
                        $(".dic_search_dic_div_first").children().remove();
                        $(".dic_search_dic_div_first").append(result)
                    }
        ...
        if ($(".route-join").size() > 0) {
            $(".course-description .route-join").click(function() {
                var id = $(this).data("id");
                if (!id) {
                    layer.msg("网络错误，请稍后重试", {icon: 2});return false
                }
                $.post("/join_route/", {id: id}, function(res) {
                    if (res.code == 1) {
                        layer.msg(res.msg, {icon: 1, time: 1800}, function() {window.location.reload()})
                    } else {
                        layer.msg(res.msg, {icon: 2})
                    }
                }, "json")
            ...
            $(document).on("click", ".route-join-all .join", function() {
                var route_param = [];
                $.each($(".route-join-list li"), function(i, val) {
                    if ($(this).is(".checed-li")) {
                        route_param.push($(this).data("id"))
                    }
                });
                if (!route_param) {
                    layer.msg("请选择需要添加的学习计划");return false
                }
                $.post("/one_join_route/", {route_param: route_param}, function(res) {
                    if (res.code == 1) {
            ...
            $(document).on("click", ".route-join-all .cancel", function() {
        ...
        $(".cart-top .cart-content .detail-box li .del").click(function() {
            var that = $(this);
            layer.confirm("确定将该课程从列表中删除吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
                var id = that.data("id");
        ...
        $("#course_class_btn_pay").click(function() {
            ...
            if (!courses) {layer.msg("课程不能为空", {icon: 2});return;}
        ...
        $(".hb_fq_pay_type").click(function() {
            ...
            if (!courses) {
                layer.msg("课程不能为空", {icon: 2});return;
            }
        ...
        $('.php-ask .centent .wrap ul .viewContact').click(function(){
            var i = $(this).data('index');
            $.post('/wenda/viewcontact/',{i:i},function(res){
                if(res.code == 0){
                    layer.alert(res.msg, {title:false,icon: 1,closeBtn: 1,skin: "layui-layer-molv",anim: 4})
                }else{
                    layer.alert(res.msg, {title:false,icon: 2,closeBtn: 1,skin: "layui-layer-molv",anim: 4})
                }
            },'json');
        });
        $('.php-ask .centent .wrap ul .viewContactSelected').click(function(){
            var i = $(this).data('index');
            layer.confirm("确定要报名吗？", {title:false,icon: 3,skin: "layui-layer-molv",btn: ["确定", "取消"]},function() {
                $.post('/resume/selected.html',{i:i},function(res){
                    if(res.code == 0){
                        layer.alert(res.msg, {title:false,icon: 1,closeBtn: 1,skin: "layui-layer-molv",anim: 4,yes:function(){
                            window.location.reload();}
                        })
                    }else{
                        layer.alert(res.msg, {title:false,icon: 2,closeBtn: 1,skin: "layui-layer-molv",anim: 4})
        ...
        $('.askResumeConsel').click(function(){
            var i = $(this).data('index');
            layer.confirm("确定要选择他接单吗？", {title:false,icon: 3,skin: "layui-layer-molv",btn: ["确定", "取消"]},function() {
                $.post('/partjob/selected.html',{i:i},function(res){
                    if(res.code == 0){
                        layer.alert(res.msg, {title:false,icon: 1,closeBtn: 1,skin: "layui-layer-molv",anim: 4})
                    }else{
                        layer.alert(res.msg, {title:false,icon: 2,closeBtn: 1,skin: "layui-layer-molv",anim: 4})
        ...
        if($('.php-course-info-text').height() > 65){
            $('.php-course-info-open').show();
        }
        $('.php-course-info-open').click(function(){
            if($(this).is('.isOpen')){
                $(this).removeClass('isOpen');
                $('.php-course-info-text').removeClass('maxNone');
                $(this).html('展开 <i class="layui-icon layui-icon-down"></i>');
            }else{
                $(this).addClass('isOpen');
                $('.php-course-info-text').addClass('maxNone');
...
function load_more(obj) {
    ...
    $.get("/index/get_more.html", {"cat_id": cat_id, "p": page}, function(res) {
...
function vip_join_group() {
    layer.msg("VIP会员默认加入该组，不可手动加入或退出", {icon: 2})
}
function ask_reply_good(id) {
    if ($("#login").size() > 0) {
        $(document).trigger("api.loginpopbox");return false
    }
    $.post("/wenda/good.html", {id: id}, function(result) {
        if (result.code == 1) {
            layer.msg(result.msg, {icon: 1})
        } else {
            layer.msg(result.msg, {icon: 2})
...
function ask_comment_good(id) {
    ...
    if (id == "") {return false}
    if (is_ask_good_post == true) {return false}
    is_ask_good_post = true;
    $.post("/wenda/follow.html", {id: id}, function(result) {
        if (result.code == 1) {
            layer.msg(result.msg, {icon: 1, time: 1200}, function() {window.location.reload()})
        } else {
            layer.msg(result.msg, {icon: 2});is_ask_good_post = false
        }
    })
}
function category_join_group() {
    ...
    $.post("/wenda/ajax_follow.html", {cat_id: cid, cat_fid: cfid}, function(result) {
...
function video_play(video_path, timestamp, seek) {
    ...
    function timeHandler() {
        if (is_tPost && timestamp && $(".task_right").is(":hidden") && (player.hasOwnProperty("time") && (player.time > timestamp) || player.hasOwnProperty("video") && (player.video.currentTime > timestamp))) {
            is_tPost = false;
            $.post("/course/video_task.html", {learn: learn, timestamp: timestamp}, function(result) {
                if (result.status == 1) {
                    if (result.task != 1 && result.is_lock == 1) {
                        // try {player.pause()} catch(e) {player.videoPause()}
                        layercloseBtn = false;
                        layerlock = true
                    }
                    if (result.task == 0) {
                        ...
                        if (result.type == 1) {
                            layer.msg(result.msg, {icon: 3}, function() {
                                is_tPost = layerlock;
                            ...
                            if (result.type == 2) {
                                ...
                                if (result.type == 3) {
                                    // layer.msg(result.msg + "?<br/>(请正确回答以上问题)<br/>即将进入问答界面...", {icon: 3,time:1800}, function() {
                                        is_tPost = layerlock;
                                        layer.open({
                                            type: 1,
                                            area: "380px",
        ...
        if (is_record) {
            if(player.hasOwnProperty("video")){
                $.post("/course/record.html", {learn:learn,type:1,dur:parseInt(player.video.duration)}, function() {});
            }else{
                $.post("/course/record.html", {learn:learn,type:1,dur:parseInt(player.duration())}, function() {});
            }
            is_record = false
        }
    }
    function endedHandler() {
        ...
        // if (url.indexOf("javascript") != -1) {
            $(".course_code_next_tips_second h2").html("厉害了word哥，本节课程已经看完了！");
        ...
        $.post("/course/record.html", {learn: learn, type: 2}, function() {})
    }
    if (is_login == 0) {
...
function wei_pay(type) {
    if ($("#login").size() > 0) {$("#login").trigger("click");return false}
    if (type == 0) {
        pay_type(type)
    } else {
        $.post("/pay_transfer.html", {type: type}, function(result) {
            if (result.code == 1) {
                ...
                if (result.code == 2) {
                    layer.msg(result.msg, {icon: 7, time: 1200}, function() {
                        enroll(0, type)
                    })
                } else {
                    layer.msg(result.msg, {icon: 2})
...
function pay_type(type, params) {
    if ($("#login").size() > 0) {$("#login").trigger("click");return false}
    if (!type) {
        layer.msg("参数错误，请联系管理员", {icon: 2});return false
    }
...
function publish_article(id) {
    if ($(".php-user #login").size() > 0) {$("#login").trigger("click");return false}
...
function publish_article_send() {
    ...
    if (!reg.test(content) || content.length < 100) {
        layer.msg("内容不可纯英文，且至少100字以上", {icon: 2});return false
    }
    ...
    if (catId == "") {
        layer.msg("请选择分类", {icon: 2});return false
    }
    if (pic == "") {
        layer.msg("请上传图片", {icon: 2});return false
    }
    if (isArticlePublishPost == true) {return false}
    isArticlePublishPost = true;
    $.post("/article/add.html", {title: title, content: content, catid: catId, verify: verify, pic: pic, task_id: task_id, ArtId: ArtId}, function(result) {
        if (result.status == 1) {
            layer.msg(result.msg, {icon: 1}, function() {
                if (result.url) {
...
function publish_ask(this_html, type, task,index) {
    if ($(".php-user #login").size() > 0) {$("#login").trigger("click");return false}
...
function publish_ask_send(type, task) {
    ...
    var index = $(".askindex").val();
    var phpcn = $("input[name='phpcn']").val();
    var resume_status = $("input[name='resume_status']:checked").val();
    ...
    switch(type){
        ...
        case 2:
            ...
            break;
        case 3:
            var publish_url = "/resume/add.html";
            break;
        case 4:
            var publish_url = "/resume/add.html";
            break;   
    ...
    if (!reg.test(content) || content.length < 50) {
        layer.msg("内容不可纯英文,且字数至少50字以上", {icon: 2});return false
    }
    if (!new RegExp("[\\u4E00-\\u9FFF]+", "g").test(title) || title.length < 5) {
        layer.msg("标题不可纯英文,且字数至少5个字以上", {icon: 2});return false
    }
    ...
    if (catIds.length <= 0) {
        layer.msg("请选择语言分类", {icon: 2});return false
    }
    /*if (catIds.length > 3) {
        layer.msg("最多选择三个分类", {
            icon: 2
    ...
    function(result) {
        if (result.status == 1) {
            layer.msg(result.msg, {icon: 1,time:1200}, function() {
                if (result.url) { parent.location.href = result.url}
            })
        } else {
            refreshVerify(".js-verify-refresh");
            layer.msg(result.msg, {icon: 2});
...
function course_vip() {
    layer.alert("内部课程，禁止观看", {title: "温馨提示", icon: 2, skin: "layui-layer-molv", closeBtn: 0, anim: 4})
}
try {
...
function searchs() {
    ...
    if (key_words == "") {
        layer.msg("请输入关键词", {icon: 2, offset: "t", anim: 6});return false
    }
...
function cancel_plan(id) {
    if (!id) {
        layer.msg("网络错误，请刷新后重试", {icon: 2});return false
    }
    var isPost = false;
    layer.confirm("确定要取消该学习计划吗？", {icon: 3,skin: "layui-layer-molv",btn: ["确定", "取消"]},function() {
        if (isPost == true) {return false }
        isPost = true;
        $.post("/cancel_plan/", {id: id},function(res) {
            if (res.code == 1) {
                layer.msg(res.msg, {icon: 1,time: 2000},function() {window.location.reload()})
            } else {
                isPost = false;
...
function dic_search() {
    ...
    $.get("/dic_search.html", {key: key}, function(result) {
        if (result == 0) {
            layer.msg("操作太频繁，请登录后再尝试", {icon: 2, offset: "t", anim: 6}, function() {
                $(document).trigger("api.loginpopbox");return false
            })
        } else {
            if (result == 2) {
                layer.msg("操作太频繁，请两分钟后再尝试");return false
            } else {
                if (result == 3) {
                    layer.msg("搜索不能为空");return false
                } else {
                    if (result == 1) {
...
function has_scrollbar() {
    if ($(".icon-clock").size() > 0) {
        $(".nano-content").animate({scrollTop: $(".icon-clock").offset().top - 200}, 1000)
    }
}
...
function saveTopDraft() {
    layer.confirm("保存草稿箱将覆盖上一次保存的内容，是否继续？", {
        ...
        yes: function() {
            ...
            if (!data.content) {
                layer.msg("内容不能为空", {"icon": 2, "anim": 6});return false
            }
            $.post("/blog/admin/blog_article_draft/", data, function(res) {
                if (res.code > 0) {
...
function saveTipsDraft() {
    ...
    $.post("/blog/admin/blog_article_draft/", data, function(res) {}, "json")
}
function saveDraft() {
    layer.confirm("保存草稿箱将覆盖上一次保存的内容，是否继续？", {
        title: "温馨提示",
...
function BlogArticleDeletes(aid) {
    layer.confirm("确定要删除吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/article_del.html", {"aid": aid}, function(res) {
            if (res.code > 0) {
                layer.msg(res.msg, {"icon": 2, "anim": 6})
            } else {
                layer.msg(res.msg, {"icon": 1});
...
function BlogArticleSearchs() {
    ...
    if (wd == "") {
        layer.msg("请输入搜索关键词", {"icon": 2, "anim": 6, "offset": "t"});return
    }
    window.location.href = "?title=" + wd
}
var isBlogArticleSavePost = false;
function BlogArticleSave() {
    ...
    data.fid = $("#fid").val();
    data.type = $("#type").val();
	data.phpcn = $("input[name='phpcn']").val();
    if (window.parent.testEditor == undefined) {
        data.content = $.trim(window.parent.ue.getContent())
    } else {
        data.mcontent = $.trim(window.parent.testEditor.getMarkdown());
    ...
    data.sort = $.trim($('input[name="sort"]').val());
    if (data.title == "") {
        layer.msg("请输入文章标题", {"icon": 2, "anim": 6});return
    }
    if (data.fid == "0") {
        layer.msg("请选择文章标签", {"icon": 2, "anim": 6});return
    }
    if (data.content == "") {
        layer.msg("请输入文章内容", {"icon": 2, "anim": 6});return
    }
    if (isBlogArticleSavePost == true) {return false}
    isBlogArticleSavePost = true;
    $.post("/blog/admin/article_save.html", data, function(res) {
        if (res.code > 0) {
            layer.msg(res.msg, {"icon": 2, "anim": 6});isBlogArticleSavePost = false
        } else {
            layer.msg(res.msg, {"icon": 1});
            setTimeout(function() {
                if (data.aid) {
...
function ArticleRecovery(aid) {
    layer.confirm("确定要恢复吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/article_up.html", {"aid": aid}, function(res) {
            if (res.code > 0) {
                layer.msg(res.msg, {"icon": 2, "anim": 6})
            } else {
                layer.msg(res.msg, {"icon": 1});
...
function Recoverydeletes(aid) {
    layer.confirm("确定要彻底删除吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/article_alldel.html", {"aid": aid}, function(res) {
            if (res.code > 0) {
                layer.msg(res.msg, {"icon": 2, "anim": 6})
            } else {
                layer.msg(res.msg, {"icon": 1});
...
function ArticleCommentdel(comment_id) {
    layer.confirm("确定要删除此评论吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/comment_del.html", {"comment_id": comment_id}, function(res) {
            if (res.code > 0) {
                layer.msg(res.msg, {"icon": 2, "anim": 6})
            } else {
                layer.msg(res.msg, {"icon": 1});
...
function ACommentrecovery(comment_id) {
    layer.confirm("确定要恢复些评论吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/up_comment.html", {"comment_id": comment_id}, function(res) {
            if (res.code > 0) {
                layer.msg(res.msg, {"icon": 2, "anim": 6})
            } else {
                layer.msg(res.msg, {"icon": 1});setTimeout(function() {window.location.reload()}, 1000)
            }
        }, "json")
    })
}
function ACommentdel(comment_id) {
    layer.confirm("确定要彻底删除此评论吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/comment_alldel.html", {"comment_id": comment_id}, function(res) {
            if (res.code > 0) {
                layer.msg(res.msg, {"icon": 2, "anim": 6})
            } else {
                layer.msg(res.msg, {"icon": 1});
...
function BlogCatdel(cid) {
    layer.confirm("确定要删除此栏目吗？", {icon: 3, skin: "layui-layer-molv", btn: ["确定", "取消"]}, function() {
        $.post("/blog/admin/category_del.html", {"cid": cid}, function(res) {
            if (res.code > 0) {
...
function custom_editor() {
    UE.registerUI("btn_code", function(editor, uiName) {
